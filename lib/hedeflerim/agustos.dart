...
class AgustosWeeklyPlanPage extends StatefulWidget {
  const AgustosWeeklyPlanPage({Key? key}) : super(key: key);

  @override
  _AgustosWeeklyPlanPageState createState() => _AgustosWeeklyPlanPageState();
}

class _AgustosWeeklyPlanPageState extends State<AgustosWeeklyPlanPage> {
  ...
  Future<void> _saveWeek(int week) async {
    // Sadece ilgili haftayı güncelle
  ...
  void _addTopic(String topic, int weekIndex, String type) async {
    ...
    await _saveWeek(weekIndex); // Sadece ilgili haftayı güncelle
  }
  ...
  void _removeTopic(String topic) async {
    ...
    // Her hafta için ayrı ayrı kaydet
    for (var i = 0; i < 4; i++) {
      await _saveWeek(i);
    }
  }
  ...
  Widget build(BuildContext context) {
    if (isLoading) {
      return Scaffold(
        appBar: AppBar(
          title: Text('Ağustos Haftalık Planlama'),
