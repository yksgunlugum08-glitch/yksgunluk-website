...
class FizikChartPage extends StatefulWidget {
  @override
  _FizikChartPageState createState() => _FizikChartPageState();
}

class _FizikChartPageState extends State<FizikChartPage> {
  List<_ChartData> chartData = [];
  TooltipBehavior _tooltipBehavior = TooltipBehavior(enable: true);
  int _currentTabIndex = 4; // Fizik için doğru başlangıç değeri
  bool _loading = true;
  ...
  /// Firestore'dan fizik verilerini yükler
  Future<void> _loadChartDataFromFirebase() async {
    ...
    try {
      ...
      for (var doc in snapshot.docs) {
        ...
        if (data.containsKey('fizik')) {
          final fizik = data['fizik'];
          veri.add(_ChartData(
            data['date'] ?? "",
            (fizik['dogru'] ?? 0).toDouble(),
            (fizik['yanlis'] ?? 0).toDouble(),
            (fizik['bos'] ?? 0).toDouble(),
            (fizik['net'] ?? 0).toDouble(),
          ));
        }
      }
      setState(() {
      ...
      Future.delayed(Duration(milliseconds: 100), () {
        ...
        switch (index) {
          ...
          case 5:
            ...
            break;
          default:
            return; // Fizik için hiçbir şey yapma
        }
          ...
          PageRouteBuilder(
            ...
            transitionsBuilder: (_, animation, __, child) {
              final offsetAnimation = Tween<Offset>(
                begin: index > _currentTabIndex ? Offset(1.0, 0.0) : Offset(-1.0, 0.0),
                end: Offset.zero,
              ).animate(animation);
  ...
  Widget build(BuildContext context) {
    ...
    final String title = _isTeacherMode 
        ? "${_studentName} - Fizik Net Grafiği"
        : "Fizik Net Grafiği";
        
    return Scaffold(
      appBar: AppBar(
        title: Text(title),
        backgroundColor: const Color.fromARGB(255, 134, 0, 175),
        leading: IconButton(
                    ...
                    children: [
                      ...
                      AnimatedPositioned(
                        ...
                        child: Container(
                          ...
                          color: const Color.fromARGB(255, 134, 0, 175),
                        ),
                      ),
                    ],
                  ),
                ...
                Expanded(
                  child: chartData.isEmpty
                      ? Center(
                          child: Text(
                            _isTeacherMode
                                ? "Bu öğrencinin fizik deneme verisi bulunmuyor."
  ...
  double _getTabIndicatorPosition(BuildContext context, int index) {
    final double buttonWidth = MediaQuery.of(context).size.width / 5.75;
