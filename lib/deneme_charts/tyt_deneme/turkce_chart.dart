...
class TurkceChartPage extends StatefulWidget {
  @override
  _TurkceChartPageState createState() => _TurkceChartPageState();
}

class _TurkceChartPageState extends State<TurkceChartPage> {
  List<_ChartData> chartData = [];
  TooltipBehavior _tooltipBehavior = TooltipBehavior(enable: true);

  int _currentTabIndex = 1; // Türkçe için doğru başlangıç değeri
  bool _loading = true;
  ...
  /// Firestore'dan Türkçe verilerini yükler
    ...
    try {
      ...
      for (var doc in snapshot.docs) {
        ...
        if (data.containsKey('turkce')) {
          final turkce = data['turkce'];
          veri.add(_ChartData(
            data['date'] ?? "",
            (turkce['dogru'] ?? 0).toDouble(),
            (turkce['yanlis'] ?? 0).toDouble(),
            (turkce['bos'] ?? 0).toDouble(),
            (turkce['net'] ?? 0).toDouble(),
          ));
        }
      }
      setState(() {
      ...
      Future.delayed(Duration(milliseconds: 100), () {
        ...
        switch (index) {
          ...
          case 5:
            ...
            break;
          default:
            return; // Türkçe için hiçbir şey yapma
        }
  ...
  Widget build(BuildContext context) {
    ...
    final String title = _isTeacherMode 
        ? "${_studentName} - Türkçe Net Grafiği"
        : "Türkçe Net Grafiği";
                ...
                Expanded(
                  child: chartData.isEmpty
                      ? Center(
                          child: Text(
                            _isTeacherMode
                                ? "Bu öğrencinin türkçe deneme verisi bulunmuyor."
  ...
  double _getTabIndicatorPosition(BuildContext context, int index) {
    final double buttonWidth = MediaQuery.of(context).size.width / 4.9;
