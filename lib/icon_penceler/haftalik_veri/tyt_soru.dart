...
class TYTHaftalik extends StatefulWidget {
  // Öğretmen modu için parametreler
  final String? studentId;
  final String? studentName;

  const TYTHaftalik({Key? key, this.studentId, this.studentName}) : super(key: key);

  @override
  _TYTHaftalikState createState() => _TYTHaftalikState();
}

class _TYTHaftalikState extends State<TYTHaftalik> with SingleTickerProviderStateMixin {
  List<ChartSampleData> _chartData = [];
  List<VerimlilikData> _verimlilikData = [];
  final List<String> _dersler = ['Matematik', 'Türkçe', 'Sosyal', 'Fen'];
  bool _loading = true;
  int _touchedIndex = -1;
  ...
  GrafikTab activeTab = GrafikTab.tytSoru;

  // Geliştirilmiş yüksek ayrım düzeyli renk paleti
  final List<Color> _colorList = [
    Color(0xFF0066FF), // Mavi - Matematik
    Color(0xFF00CC00), // Yeşil - Türkçe
    Color(0xFFFF8C00), // Turuncu - Sosyal
    Color(0xFF9900FF), // Mor - Fen
  ];
  ...
  void initState() {
    ...
    print("=== 📊 TYT Haftalık Başlatılıyor ===");
  ...
  Future<void> _verileriYukle() async {
    ...
    try {
      ...
      // 1. TYT Soru Verileri - Haftalık soru sayıları (Orijinal grafiğimiz)
  ...
  Future<void> _loadSoruVerileri(String userId) async {
    try {
      ...
      for (var doc in snapshot.docs) {
        ...
        // TYT dersleri için verileri çıkar
        int matematik = (data['Matematik'] ?? 0) is int
            ? data['Matematik']
            : int.tryParse(data['Matematik']?.toString() ?? '0') ?? 0;
        
        int turkce = (data['Türkçe'] ?? 0) is int
            ? data['Türkçe']
            : int.tryParse(data['Türkçe']?.toString() ?? '0') ?? 0;
        
        int sosyal = (data['Sosyal'] ?? 0) is int
            ? data['Sosyal']
            : int.tryParse(data['Sosyal']?.toString() ?? '0') ?? 0;
        
        int fen = (data['Fen'] ?? 0) is int
            ? data['Fen']
            : int.tryParse(data['Fen']?.toString() ?? '0') ?? 0;
        
        List<int> dersVeri = [matematik, turkce, sosyal, fen];
        int toplam = matematik + turkce + sosyal + fen;
      ...
      print("✅ ${_chartData.length} haftalık TYT soru verisi yüklendi");
    } catch (e) {
  ...
  Future<Map<String, Map<String, int>>> _getHaftalikSoruVerileri(String userId) async {
    ...
    try {
      ...
      print("📊 Firestore'dan ${snapshot.docs.length} TYT belgesi alındı");
      ...
      for (var doc in snapshot.docs) {
        ...
        // TYT derslerine göre verileri ekle
        int matematik = (data['Matematik'] ?? 0) is int
            ? data['Matematik']
            : int.tryParse(data['Matematik']?.toString() ?? '0') ?? 0;
        ...
        // Mevcut değere ekle
        haftalikVeri[label]!['Matematik'] = (haftalikVeri[label]!['Matematik'] ?? 0) + matematik;
        haftalikVeri[label]!['Türkçe'] = (haftalikVeri[label]!['Türkçe'] ?? 0) + turkce;
        haftalikVeri[label]!['Sosyal'] = (haftalikVeri[label]!['Sosyal'] ?? 0) + sosyal;
        haftalikVeri[label]!['Fen'] = (haftalikVeri[label]!['Fen'] ?? 0) + fen;
      }
      
      return haftalikVeri;
    } catch (e) {
      print("❌ TYT Soru verileri alınırken hata: $e");
      return {};
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final String title = _isTeacherMode 
        ? "${_currentStudentName ?? 'Öğrenci'} - TYT İstatistikleri" 
        : "TYT Soru İstatistikleri";
                    ...
                    children: [
                      ...
                      Expanded(
                        child: _TabButon(
                          ...
                          onTap: () => _goToTab(GrafikTab.aytSoru),
                        ),
                      ),
                      const SizedBox(width: 4),
                      Expanded(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 16),
          Text(
            "Haftalık TYT soru verisi bulunamadı",
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            _isTeacherMode ? "${_currentStudentName ?? 'Öğrenci'} için henüz TYT verisi yok!" : "Henüz TYT verisi yok!",
          ...
          Padding(
            ...
            child: Text(
              _isTeacherMode ? "Öğrenci TYT soru çözdükçe burada istatistiklerini göreceksiniz" 
                           : "TYT soru çözdükçe burada istatistiklerinizi göreceksiniz",
              style: TextStyle(fontSize: 16, color: Colors.grey.shade600),
              textAlign: TextAlign.center,
            ),
          ),
          ...
          Row(
            ...
            children: [
              Column(
                ...
                children: [
                  Text(
                    _isTeacherMode ? "${_currentStudentName ?? 'Öğrenci'}" : 'TYT Soru İstatistikleri',
                    style: TextStyle(
                      fontSize: 20,
